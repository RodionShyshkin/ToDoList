...
TEST_F(TaskDTOTest, Constructor) {
  ...
  EXPECT_NO_THROW(TaskDTO::create(TaskID(1), Task::create("Task", "test", Priority::LOW, Date(2019, 12, 31)), true));
}

TEST_F(TaskDTOTest, Getters) {
  auto check = TaskDTO::create(TaskID(1), Task::create("Task", "test", Priority::LOW, Date(2008, 3, 8)), true);
