...
TEST_F(TaskDTOTest, Constructor) {
  ...
  EXPECT_NO_THROW(TaskDTO(TaskID(1), Task("Task", "test", Task::Priority::LOW, DateTime(2019, 12, 31)), true));
}

TEST_F(TaskDTOTest, Getters) {
  auto check = TaskDTO(TaskID(1), Task("Task", "test", Task::Priority::LOW, DateTime(2008, 3, 8)), true);
  ASSERT_EQ(check.getID(), 1);
  ASSERT_EQ(check.getName(), "Task");
  ASSERT_EQ(check.getLabel(), "test");
  ASSERT_EQ(check.getPriority(), Task::Priority::LOW);
