...
#include "../src/MemoryModel/TaskEntity.h"
...
/*  auto checkTask = TaskEntity(Task("Task", "test", Task::Priority::EMPTY, DateTime(2020, 10, 10, 10, 10)), 1);
  auto checkSubtask = TaskEntity(Task("Subtask", "test", Task::Priority::EMPTY, DateTime(2020, 10, 10, 10, 10)), 2);

  auto oldSubtasks = checkTask.getSubtasks();
  checkTask.AddSubtask(std::make_shared<TaskEntity>(checkSubtask));
